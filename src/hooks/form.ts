...
export function renderDatePickerAndSwitch(
  value: Ref<DatePickerValue | null>,
  switchValue: Ref<boolean>,
  options: DatePickerProps | AllowedComponentProps = {}
) {
  return h('div', { id: 'autoExec' }, [
    createVNode(NSwitch, {
      value: switchValue.value,
      onUpdateValue: (newVal: boolean) => {
        switchValue.value = newVal
        value.value = newVal === true ? Date.now() : null
      },
    }),
    switchValue.value
      ? createVNode(
          'div',
          { id: 'autoExecDate', style: 'margin-left: 20px; display: inline-flex;' },
          [
            ,
            createVNode(NDatePicker, {
              value: value.value,
      ...
      : createVNode('h1', { style: 'margin-left: 20px; display: inline;' }, '手动执行'),
  ])
}
