...
import { DataTableColumn, NButton, NPopconfirm, NIcon, NTag } from 'naive-ui'
...
export interface userDefineActionModel {
  label: string
  show: boolean
  pop: boolean
  popText: string
...
export const useRenderAction = function (actions: TableActionModel[]) {
  const renderActions = actions.map((it) => {
    const pop = it.pop === true
    const show = it.show === false ? false : true
    const button = h(
      NButton,
      {
...
export const userDefineAction = function (actions: userDefineActionModel[]) {
  const defineActions = actions.map((it) => {
    const pop = it.pop === true
    const show = it.show === false ? false : true
    const userComponent = h(
      it.component,
      {
        ...it.attribute,
...
export const tableColumnCanClick = function (actions: any, rowData: any, showKey: string) {
  return h(
    'p',
    {
